...
// ── Prediction engine ─────────────────────────────────────────────────────────
function computePrediction(coin: CoinData, fngValue: number): Prediction {
  ...
  const avg = (arr: number[]) =>
    arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : price;
  ...
  const macdBull =
    prices.length >= 26 ? ema(prices, 12) > ema(prices, 26) : chg24 > 0;
  ...
  const techScore =
    rsiScore * 0.35 + macdScore * 0.25 + maScore * 0.2 + bollScore * 0.2;
  const volRatio = (vol / mcap) * 100;
  const volScore = Math.min(100, Math.max(0, volRatio * 10 + 50 + chg24 * 0.5));
  const momScore = Math.min(100, Math.max(0, 50 + chg24 * 1.5 + chg7 * 0.8));
  const finalScore =
    fng * 0.2 + volScore * 0.22 + techScore * 0.35 + momScore * 0.23;
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      >
        ...
        <span
          style={{
            color: v > 60 ? '#4ade80' : v < 40 ? '#f87171' : '#facc15',
            fontWeight: 600,
          }}
        >
          {v.toFixed(0)}
...
export default function Home() {
  ...
  const doFetch = useCallback(async () => {
    ...
    try {
      ...
      const fngJson: FngData = await fngRes.json();

      const map: Record<string, CoinData> = {};
      (priceJson.data as CoinData[]).forEach((c) => {
        const tok = TOKENS.find((t) => t.cg === c.id);
      ...
      setErrMsg(e instanceof Error ? e.message : 'Nepoznata greška');
  ...
  if (status === 'loading' && Object.keys(coins).length === 0) {
    return (
      <div
        style={{
          background: '#060d18',
      ...
      >
        ...
        <div style={{ fontSize: 13, color: '#4ade80' }}>
          Dohvaćam live podatke…
        </div>
      </div>
    );
  }

  if (status === 'error' && Object.keys(coins).length === 0) {
    return (
      <div
        style={{
          background: '#060d18',
        ...
        >
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 12, color: '#64748b' }}>
              Live podaci · CoinGecko + Alternative.me · Server-side proxy
            </p>
          </div>
          <div
            style={{
          ...
          >
            {fng && (
              ...
              >
                ...
                <span style={{ color: '#475569' }}>({fng.label})</span>
              </div>
            )}
            <button
              onClick={canRefresh ? doFetch : undefined}
              style={{
                background: '#0d1f2d',
                border: '1px solid #1e3a5f',
                color: canRefresh ? '#94a3b8' : '#334155',
                borderRadius: 8,
                padding: '6px 14px',
            ...
            {lastFetch && (
              <div style={{ fontSize: 10, color: '#334155' }}>
                Zadnje: {lastFetch.toLocaleTimeString('hr')}
              </div>
            )}
          </div>
        </div>
          ...
          {TOKENS.map((t) => {
            ...
            return (
              <div
                ...
                style={{
                  background: isSel ? '#0a1e35' : '#0a1422',
                  border: `1px solid ${isSel ? t.color : '#1e2d3d'}`,
                  borderRadius: 14,
                  padding: 16,
                  cursor: 'pointer',
                  transition: 'all 0.2s',
                  boxShadow: isSel ? `0 0 20px ${t.color}30` : 'none',
                }}
              >
                <div
                  style={{
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div
                      style={{
                        fontSize: 11,
                        fontWeight: 700,
                        color: sigColor(p.signal),
                        background: `${sigColor(p.signal)}15`,
                        borderRadius: 6,
                        padding: '3px 8px',
                        border: `1px solid ${sigColor(p.signal)}40`,
                      }}
                    >
                      {p.signal}
                    </div>
                ...
                >
                  <span>
                    ...
                    >
                      {p.finalScore.toFixed(0)}/100
                    </b>
                  </span>
                  <span>
                    Pouzdanost:{' '}
                    <b style={{ color: '#94a3b8' }}>{p.confidence}%</b>
                  </span>
                </div>
              </div>
            );
          ...
          (() => {
            ...
            return (
              <div
                style={{
                  background: '#0a1422',
                  border: `1px solid ${selMeta.color}50`,
                  borderRadius: 16,
                  padding: 24,
                }}
              >
                  ...
                  >
                    <span
                      style={{
                        ...
                        boxShadow: `0 0 8px ${selMeta.color}`,
                ...
                >
                  <div>
                    ...
                    <ScoreBar
                      label={`Fear & Greed (${FNG_LABEL(fngVal)})`}
                      value={fngVal}
                      color="#22d3ee"
                    />
                    <ScoreBar
                      label="Momentum 24h"
                      value={Math.min(100, Math.max(0, 50 + chg24 * 2))}
                      color="#a78bfa"
                    />
                    <ScoreBar
                      label="Momentum 7d"
                      value={Math.min(100, Math.max(0, 50 + chg7 * 1.2))}
                      color="#60a5fa"
                    />
                    <div
                    ...
                    {(
                      [
                        ...
                        [
                          'Vol/MCap',
                          c.market_cap
                            ? ((c.total_volume / c.market_cap) * 100).toFixed(
                                2
                              ) + '%'
                            : '—',
                        ],
                  ...
                  <div>
                    ...
                    {(
                      [
                        ...
                        [
                          'MACD',
                          p.macdBull ? 'Bullish ▲' : 'Bearish ▼',
                          p.macdBull ? '#4ade80' : '#f87171',
                        ],
                        [
                          'MA7 vs MA30',
                          p.ma7 > p.ma30 ? 'Golden Cross ▲' : 'Death Cross ▼',
                          p.ma7 > p.ma30 ? '#4ade80' : '#f87171',
                        ],
                        [
                          'Bollinger',
                          `${p.bollPct}%`,
                      ...
                      >
                        <span>{k}</span>
                        <span style={{ color: cl, fontWeight: 600 }}>{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                >
                  ⚠️ <strong style={{ color: '#f59e0b' }}>Upozorenje:</strong>{' '}
